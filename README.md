...
エージェントは自走する。人間は要所で手綱だけを握る。**Tazna** は流れの中から「いま人間が判断すべき一点」だけを検知し、俯瞰（マップ）から1ビュー1判断（判断カード）へ畳んで差し出す。

![スプリント上の現在地と halt 理由を示すマップ](./docs/tazna-map.png)

*マップ：スプリント上の「現在地」と、なぜ止まっているか（halt 理由）を1画面で。依存の位相は GitHub から読み、その上の現在地だけを示す（依存グラフは所有せず読む）。*

![auto-merge 判断を論点・推奨・トレードオフに畳んだ判断カード](./docs/tazna-decision.png)

*判断カード：実行工程（4/7 完了）とともに、いま決めるべき一点を提示。論点・現状/前提・推奨（ADR 根拠つき）・2 択の可逆性とトレードオフまで畳み、人間が文脈を再構築せずに即断できる形にする。単なる可視化（observability）ではなく、判断のための圧縮に徹する。*
